...
//Burda Yasemin Bebeğin fotisi olmalı onu unutmayalımm
class DoctorValues extends StatefulWidget {
  const DoctorValues({Key? key}) : super(key: key);
...
class _DoctorValuesState extends State<DoctorValues> {
  ...
  Padding buildNoteInputBox() {
    return Padding(
      padding: EdgeInsets.only(left:12.w,top: 2.h,right: 32),
      child: TextField(
        controller: _noteController,
        keyboardType: TextInputType.text,
        maxLines: null, // İstediğiniz kadar satır girebilirsiniz.
        textAlignVertical: TextAlignVertical.top, // Metni yukarıya hizala
        decoration: InputDecoration(
          hintText: '',
          filled: true,
          fillColor: Colors.white,
          contentPadding: EdgeInsets.only(left: 5.w,top: 30.h,right: 22,bottom: 5.h),
  ...
  Widget buildAddPercentilButton() {
    return RoundedButton(
      ...
      onPressed:(){
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const DoctorPercentilValues()),
        );
      },
          );
  }
  ...
  Widget buildValueButton() {
    return RoundedButton(
        ...
        onPressed: (){Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => BabyValues()),
        );}
            );
  }
